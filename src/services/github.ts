...
export class GitHubService {
  ...
  async createLocalizationBranch(
    ...
    try {
      ...
      const translationFiles = files.filter(f => f.type === 'translation');
      let stringsCount = 0;
      let languagesList: string[] = [];
      if (translationFiles.length > 0) {
        try {
          const parsed: any = JSON.parse(translationFiles[0].content);
          languagesList = Object.keys(parsed || {});
          const firstLang = languagesList[0];
          if (firstLang && parsed[firstLang] && typeof parsed[firstLang] === 'object') {
            stringsCount = Object.keys(parsed[firstLang]).length;
          }
          // Fallback if consolidated format isn't used
          if (stringsCount === 0) {
            const keys = Object.keys(parsed || {});
            stringsCount = keys.length;
          }
        } catch (e) {
          console.warn('Failed to parse translation file for PR summary:', e);
        }
      }
      const languagesSection = (languagesList && languagesList.length)
        ? languagesList.map(l => `- ${l}`).join('\n')
        : translationFiles.map(f => `- ${f.path}`).join('\n');
